                    ...
                    <tbody class="main">
                        <?php
                            foreach ($data as $order_id => $value) {
                                ...
                                // 操作字符串
                                if($value['status'] == 1) {
                                    $shouhuo = ($value['send_status'] != 2) ? "<a onclick=\"return confirm('请确认');\" href='".URL('Admin/Myorder/shouhuo?order_id='.$order_id)."'>已签收</a>" : '';    
                                } else {
                                    $shouhuo = '';
                                }
                                if($value['send_status'] == 0) {
                                    $quxiao = ($value['status'] == 1) ? "<a onclick=\"return confirm('请确认');\" href='".URL('Admin/Myorder/quxiao?order_id='.$order_id)."'>取消订单</a>" : '';
                                } else {
                                    $quxiao = '';
                                }
                                
                                $caozuo = "<td>$shouhuo | $quxiao</td>";
